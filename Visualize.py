...
def User_Class(user, dict=Users_Classified, key="Heuristic Class"):
    if str(user) in dict.keys():
        if key in dict[str(user)].keys():
            return(dict[str(user)][key])
        else:
            return("Unknown")
    else:
        return ("Unknown")
...
def Users_Classlist(tree, dict=Users_Classified, key="Heuristic Class", Details=False):
    userset = set()
    usertypeset = set()
    for node in tree.nodes:
        userset.add(tree.nodes[node]['user'])
    for user in userset:
        usertypeset.add(User_Class(user))
    if Details:
